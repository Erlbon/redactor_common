...
| `standard_shortcuts.py` | Canonical shortcut-string constants (`LOAD_FILES`, `SAVE_AS`, `RENAME_SINGLE_FILE`, `PARSE_FILENAME_TO_METADATA`, `REDO`, `HELP`, ...) for every action shape all four apps share, matching Qt's own `QKeySequence::StandardKey` Windows bindings where one exists (verified via `QKeySequence.keyBindings()`, not assumed) — a project imports these instead of repeating literal key strings. See its own module docstring for the full rationale and the 2026-09-13 audit that produced it (mp3 missing Ctrl+O entirely, Parse Filename squatting on F3, three different "Save As" keys, videoredactor's Exit bound to a StandardKey that doesn't work on Windows) | new, 2026-09-13, not yet consumed by any project |
